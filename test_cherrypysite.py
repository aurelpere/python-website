"""
this is a test file for cherrypy website site.py
"""
#!/usr/bin/python3
# coding: utf-8
import os
...
import requests
import cherrypy
from cherrypysite import MonSiteWeb
from cherrypysite import Questions
from cherrypysite import Code
from cherrypysite import Photos
from cherrypysite import Music
from cherrypysite import Publis
from cherrypysite import NonViolence
from cherrypysite import Meta
from cherrypysite import launch_website
...
def test_launchwebsite():
    "test launchwebsite function for cherrypy"
    def kill_child_processes(parent_pid, sig=signal.SIGTERM):
        "kill pid process function (for information in case of os.system use)"
        try:
        ...
        for process in children:
            ...
            # see https://psutil.readthedocs.io/en/latest/#kill-process-tree for process tree

    def fetch(url):
        req = requests.get(url)
        return (req.status_code,html.unescape(req.text))
    with cf.ThreadPoolExecutor(max_workers=2) as executor:
        print('future website launch')
        future_website = executor.submit(launch_website)
        print('future website launched')
        time.sleep(3)
        print('future request launch')
        future_request = executor.submit(fetch, 'http://127.0.0.1:25638')
        print('future request launched')
        future_collection = future_request, future_website
        time.sleep(2)
        # other method alternative to concurrent.futures is pebble here
        # pebble seems to be able to cancel processes shorturl.at/kstuM
        done, not_done = cf.wait(future_collection, timeout=5, return_when=cf.FIRST_COMPLETED)
        print(f'done: {done}; not done: {not_done}')
        if future_request in cf.as_completed(future_collection,timeout=None):
            print('future request in done')
            print(future_request.result()[0])
            assert future_request.result()[0] == 200
            print('assert 200 ok ')
            assert future_request.result()[1] == """
<!DOCTYPE HTML PUBLIC "-//W3C//DTD HTML 4.0//EN">
<HTML>
<TITLE>The ꘏꘏�꘏꘏ website</TITLE>
<HEAD>The ꘏꘏�꘏꘏ website</HEAD>
...
contact : aurel.pere [at] gmail.com
        ...
        executor.shutdown(wait=False)#not necessary because we used "with" above
        kill_child_processes(os.getpid())#not necessary but useful if launched with os.system
